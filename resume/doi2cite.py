...
class DOI:
    ...
    def j_inits(self):
        ...
        j = j.split(' ')
        j = [w[0] for w in j if w[0].isupper()]
        j = ''.join(j)

        return j

    def a_inits(self, emph=''):

        al = self.author_list().replace(' and','').replace(',','')
        al = al.replace('. ','').replace('.','').split(' ')
        al = [''.join([c for c in w if c.isupper()]) for w in al]

        if al[0] == emph:
            return f'{al[0]} et al.'
        elif al[1] == emph:
            return f'{al[0]}, {al[1]} et al.'
    ...
    def cited_by(self):
        return self.info['is-referenced-by-count']
    ...
    def __init__(self,doi,silent=False,arxiv=False,output='tex',links=False,
        ...
        if arxiv:
            ...
            url_cr = f'https://api.crossref.org/works/{DOI.arxiv_doi(doi):s}'
        else:
            url    = f'https://doi.org/{doi:s}'
            url_cr = f'https://api.crossref.org/works/{doi:s}'
        ...
        if r_cr.status_code != 200:
            print(f'failed with code {r_cr.status_code:d} ({r_cr.text:s})')
            sys.exit(-1)



        self.info = r.json()
        self.info_cr = r_cr.json()['message']
...
#            return f'arXiv: {arxiv_num:s}'
    ...
    def title(self,link=False,maxc=-1):
        ...
        # if maxc is set, truncate title after maxc words
        if maxc > 0:
            words = title.split(' ')
            if len(words) > maxc:
               title = ' '.join(words[:maxc]) + ' ...'
    ...
    def year(self):
        return self.info['issued']['date-parts'][0][0]

    def citation(self):

        auths = self.author_list()

        title = self.title(link=self.links)
